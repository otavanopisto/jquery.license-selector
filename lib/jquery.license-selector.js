  ...
  $.widget("custom.licenseSelector", {
    options : {
      types: {
        'cc-4.0': {
          ...
          hintTextLocale: 'creativeCommonsHintText',
          hintLinkLocale: 'creativeCommonsHintLink'
        },
        'cc-3.0': {
          strategy: 'cc',
        ...
        'ogl': {
          ...
          hintTextLocale: 'oglHintText',
          hintLinkLocale: 'oglHintLink'
        },
        'link': {
          strategy: 'url',
          defaultValue: '',
      ...
      locales: {
        en: {
          ...
          'creativeCommonsHintText': 'About Creative Commons License',
          'creativeCommonsHintLink': 'https://creativecommons.org/licenses/',
          'oglHintText': 'About Open Game License',
          'oglHintLink': 'https://en.wikipedia.org/wiki/Open_Game_License',
          'linkHintText': 'Enter a link to your license'
        },
        fi: {
          'createCommons30Label': 'Creative Commons 3.0', 
          'createCommons40Label': 'Creative Commons 4.0', 
          ...
          'createCommonsCommercial': 'Sallitaanko teoksen kaupallinen käyttö?',
          'createCommonsCommercialYes': 'Kyllä',
          'createCommonsCommercialNo': 'Ei',
          'cc0Label': 'CC0 1.0 Yleismaailmallinen',
          'noneLabel': 'Ei lisenssiä',
          'creativeCommonsHintText': 'Tietoja Creative Commons -lisenssistä',
          'creativeCommonsHintLink': 'http://creativecommons.fi/lisenssit/',
          'oglHintText': 'Tietoja Open Game License -lisenssistä (englanniksi)',
          'oglHintLink': 'https://en.wikipedia.org/wiki/Open_Game_License',
          'linkHintText': 'Anna linkki lisenssin'
        }
      }
    },
    ...
    val: function (url) {
      if (url !== undefined) {
        ...
        var hintText = this._getHintText(typeId);
        if (hintText) {
          var hintLink = this._getHintLink(typeId);
          if (hintLink) {
            this._widget.find('.hint').attr('href', hintLink);
          } else {
            this._widget.find('.hint').removeAttr('href');
          }
          
          this._widget.find('.hint')
            .text(hintText)
            .show();
        } else {
          this._widget.find('.hint').hide();
        }
      } else {
    ...
    _getHintLink: function (type) {
      if (type) {
        var typeOptions = this._getTypeOptions(type);
        if (typeOptions) {
          if (typeOptions.hintLinkLocale) {
            return this._getText(typeOptions.hintLinkLocale);
          }
        }
      }
    ...
    _getHintText: function (type) {
      if (type) {
        var typeOptions = this._getTypeOptions(type);
        if (typeOptions) {
          if (typeOptions.hintTextLocale) {
            return this._getText(typeOptions.hintTextLocale);
          }
        }
      }
    ...
    _getCreativeCommonsAttributeIndex: function (attribute) {
      switch (attribute) {
        case 'by':
          return 1;
        case 'nc':
    ...
    _sortCreativeCommonsAttributes: function (attributes) {
      attributes.sort($.proxy(function (attribute1, attribute2) {
        return this._getCreativeCommonsAttributeIndex(attribute1) - this._getCreativeCommonsAttributeIndex(attribute2);
      }, this));
    ...
    _createCreativeCommonsUrl: function (objectModel) {
      return "https://"
        + CC_URL_PREFIX 
        + this._sortCreativeCommonsAttributes(objectModel.attributes).join('-')
        + '/'
