(function() {
  ...
  $.widget("custom.licenseSelector", {
    options : {
      types: {
        ...
        'text': {
          ...
          placeholderLocale: 'textPlaceholder'
        },
        'none': {
          strategy: 'none',
      ...
      locales: {
        en: {
          ...
          'linkLabel': 'Link',
          'linkPlaceholder': 'License URL address',
          'textLabel': 'Text',
          'textPlaceholder': 'License text',
          ...
          'textLabel': 'Teksti',
          'textPlaceholder': 'Lisenssiteksti',
    ...
    _create : function() {
      ...
      this.val(this.element.val(), 'none');
    ...
    val: function (url, typeOverride) {
      if (url !== undefined) {
        this.element.val(url);
        this.element.hide();
        var typeId = this._resolveType(url, typeOverride||this._widget.find('select.type').val());
  ...
  _renderAttributes: function(attributes) {
      var result = [];
      if (attributes.indexOf('by') !== -1) {
        result.push('by');
      }
      if (attributes.indexOf('sa') !== -1) {
        result.push('sa');
      }
      if (attributes.indexOf('nc') !== -1) {
        result.push('nc');
      }
      if (attributes.indexOf('nd') !== -1) {
        result.push('nd');
      }
      return result.join('-');
    },
    ...
    _createCreativeCommonsUrl: function (objectModel) {
      return "https://"
        + CC_URL_PREFIX
        + this._renderAttributes(objectModel.attributes)
        + '/'
